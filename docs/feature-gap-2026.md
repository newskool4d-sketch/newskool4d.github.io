...
## 9. T11 구형 스텁화 — 1단계(2026-09-15)

**이 작업으로 3단계 종료 게이트("구형 공개 페이지 0건")는 아직 충족되지 않는다.** 이번은 계획표의 "리다이렉트 스텁 1배포 주기 → 제거" 중 스텁 전환(1단계)만이며, 두 파일 자체를 저장소에서 제거하는 것은 1배포 주기 경과를 확인한 뒤 별도 승인을 받아 진행한다.
