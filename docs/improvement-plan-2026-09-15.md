...
| T11. 구형 스텁화 | 리다이렉트 스텁 전환. GNB·README·worker 자산 목록·`check-sensitive` 동시 갱신. **스텁 파일은 영구 존치**(2026-09-16 결정 — 아래 진행 현황) | 구형 2종, `shared.js`, `tools/build-sites-worker.mjs` | 구형 URL 정상 이동, worker 자산의 구형 **기능** 콘텐츠 0건 |
| T8-1. 폐기 토큰 | `#2a41b6`·`#4262ff`·`#0f172a` 제거, `index.html` 사문 다크 CSS 삭제, `atlas-theme.css` `!important` 의존 축소(구형 제거 시 자연 해소분 우선). 잔여 접근안은 격차표 §12-5 — 두 시트 병합이 아니라 **불필요한 `!important` 제거**(atlas가 마지막 시트라 대부분 순서만으로 이김), 범위에 `shared.css`(69건) 포함 | `unified-map.html`, `js/map-layers.js`, `shared.css`, `index.html` | 폐기 토큰 grep 0건, `!important` 건수 50% 이상 감소 |
...
| T13. DESIGN 감사 | §7 체크리스트 + §8 CI 규정 + §9 Atlas 계약 전 항목 대조 — **감사 완료(격차표 §12), 이탈 4건** | 전체 | 전 항목 "이행 완료" 표기 |
| T2-②③. 구조 정리 | 중첩 폴더 평탄화(`git mv`, 태그 선행, OneDrive 동기화 중지), 루트 빈 폴더 제거 | 래퍼 구조 | 저장소 루트 = 앱 루트 |

**게이트**: 구형 **기능** 페이지 0건(리다이렉트 스텁만 존치) + DESIGN.md 전 항목 이행 + CI 녹색 + 실 키 스모크 재수행.
...
**3단계 진행 현황 (T11 2단계 판단, 2026-09-16)**: 스텁 2개를 **삭제하지 않고 영구 존치**하기로 결정했다. 사유: `schools.html`·`infrastructure.html`은 공개 사이트에서 발행된 주소라 과거 공문·안내자료·즐겨찾기에 남아 있을 수 있는데, 삭제하면 GitHub Pages 특성상 **영구 404**가 되고 되돌릴 수 없다. 계획서 원문의 "1배포 주기 뒤 제거"에서 실제 경과는 약 1일로, 외부 유통 링크의 유예 기간으로 짧다. 반면 존치 비용은 29줄 × 2개, 유지보수 0이다. 따라서 종료 게이트의 "구형 공개 페이지 0건"을 **"구형 기능 페이지 0건(리다이렉트 스텁만 존치)"**로 재정의하고 충족 처리한다. 삭제를 택했다면 함께 정리했어야 할 항목(`tools/build-sites-worker.mjs`의 자산 2개, `README.md` 배포 범위 행, `docs/data-update-guide.md` 괄호 문구)은 존치 결정에 따라 **모두 현행 유지**한다. 판단 시점 실측: 두 스텁 URL 모두 HTTP 200, `meta refresh` + `location.replace` 정상 동작. 세부는 `docs/feature-gap-2026.md` §9.

**3단계 진행 현황 (T13 DESIGN 감사, 2026-09-16)**: `DESIGN.md` §7·§8·§9의 검증 가능한 규정을 전수 계측했다. 전체 판정표는 `docs/feature-gap-2026.md` §12. **§8(CI 사용) 전 항목 이행**, §7 마이그레이션 체크리스트 8건 전부 이행, §9 접근성·공급자·상태 계약 이행. **이탈 4건**: ① `atlas-theme.css` 라운딩 16px 초과 8종 ② 같은 파일의 §7 4단계에 매핑되지 않는 임의 그림자 약 10종 ③ 브레이크포인트 1100/1101px ≠ 계약의 1180px ④ 패널 모션 220ms 선언 부재.

감사의 핵심 발견은 **이탈이 한 파일에 모여 있다**는 것이다. `unified-map.html`의 인라인 토큰(`--um-radius-sm/md/lg` 8/12/16px, `--um-raised`, `--um-modal`)은 DESIGN과 정확히 일치하는데, 마지막에 로드되는 `atlas-theme.css`가 이를 `!important`로 덮어 DESIGN에서 **멀어지는 값**으로 바꾼다. 즉 T13 이탈(①②)과 T8-1의 `!important` 240건은 같은 원인이며 함께 처리해야 한다. ③은 계약 문구와 구현 중 무엇을 맞출지 **사용자 결정이 필요**하고(1101~1179px 구간 레이아웃이 바뀜), ④는 저위험 추가로 해소된다.

오탐 정정도 3건 있었다 — `index.html:22`의 다크 `#181716`은 `atlas-theme.css:18-26`이 `body`를 덮어 **렌더링에 도달하지 못하는 사문 선언**, `shared.css:522`의 `.loader-icon`은 사용처 0건인 **죽은 CSS**(실제로는 `shared.js:408`의 `.loader-signal` CSS 모션이 쓰임), `--teal-light`·`--blue-light`는 **값이 이미 `#0060b0`·`#00b0e0`로 교체**돼 변수명만 레거시다. 셋 다 위반이 아니라 청소 대상이다.

**3단계 종료 게이트 점검 (2026-09-16 기준)**: 4개 조건 중 **2개 충족**.

| 조건 | 상태 | 근거 |
|---|---|---|
| 구형 **기능** 페이지 0건(스텁 존치) | **충족** | 구형 기능 코드 0줄. 리다이렉트 스텁 2개는 링크 보존을 위해 영구 존치(2026-09-16 결정), 라이브 HTTP 200·리다이렉트 정상 |
| DESIGN.md 전 항목 이행 | 미충족 | **T13 감사 완료**(§12) — 이탈 4건: 라운딩 16px 초과 8종·임의 그림자 약 10종·브레이크포인트 1100↔1180·패널 모션 220ms 부재. T8-1 `!important` 42.2%(목표 50%, 접근안 §12-5·범위에 `shared.css` 69건 포함). **T12는 완료**(§11 — 대비·이중 부호화·터치 타깃·200% 확대 4항 전부 통과) |
