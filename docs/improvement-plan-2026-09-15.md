...
**3단계 진행 현황 (T12 접근성 실측, 2026-09-16)**: 파스텔 칩 대비 **20쌍 전부 AA 통과**(최소 4.71:1, `.um-popup-chip` 기본), 색+형태 이중 부호화 충족(마커 글리프·모양, 모든 칩에 텍스트 라벨 동반), 375px 터치 타깃 **결함 1건 발견·수정**(`.um-row-focus` 40px → 44px). 세부와 실측표는 `docs/feature-gap-2026.md` §11. 측정 과정에서 브라우저 계산값이 CSS 표준으로 설명되지 않아 개입 실험 3건으로 **계측 도구 결함**을 확인하고 해당 수치를 근거에서 제외했다(§11-4) — 그 수치에 근거해 잠시 제거했던 `atlas-theme.css`의 `min-height: 760px !important`는 **되돌렸다**. 모바일 지도 760px는 DESIGN 하한(`48svh`) 위반이 아니므로 결함이 아니며, 축소는 디자인 변경이라 사용자 판단으로 이월한다. **200% 확대 시 가로 스크롤 없음은 미검증**(S14와 함께 이월).
...
| DESIGN.md 전 항목 이행 | 미충족 | T13 감사 미실시. T8-1 `!important` 42.2%(목표 50%). T12 접근성 실측은 대비·이중 부호화·터치 타깃 완료(§11), 200% 확대 검증만 잔여 |
