...
Static deployment remains the baseline for page assets. Naver Maps is the default public provider and Kakao Maps remains a temporary fallback during migration. Road-route connections are the only server-assisted public feature, and **it is not part of the static baseline**: a same-origin `/api/directions` proxy may use deployment secrets to call Naver Directions 5, but that route only exists on the separate Sites worker build (`tools/build-sites-worker.mjs` → `dist/server/index.js`), never on the canonical GitHub Pages deployment. Browser pages never accept, expose, or store a Directions Client Secret. If the worker's server credential is unavailable, the rest of that deployment stays usable and the connection control reports a configuration state instead of drawing a straight-line substitute.

**On the canonical GitHub Pages deployment, road-route connections are unavailable outright** (verified 2026-09-15): a static host has no server route to answer `/api/directions`, so the request returns GitHub Pages' own 404 HTML instead of JSON, and the client-side `fetchRoadRoute()` surfaces this as a generic request failure rather than the intentional configuration state described below. This is a hosting-shape limitation, not a bug to fix in the client — do not add straight-line fallback logic to work around it. Treat road-route connections as a Sites-worker-only capability until/unless that worker becomes the canonical deployment.

The Directions proxy requires a deployment-provided `DIRECTIONS_RATE_LIMITER` binding whose `limit({ key })` state is shared across worker instances and configured for 12 requests per minute. The generated worker fails closed with `503 directions_rate_limit_not_configured` when that distributed binding is absent, so credentials alone can never enable an unmetered paid proxy. It also rejects cross-site browser requests, accepts only endpoints inside the documented greater-Incheon coordinate envelope, and aborts the upstream request after eight seconds. **This distributed-quota behavior is implemented and unit-tested (`tests/sites-worker-directions.test.mjs`) but has not been exercised against the real Sites worker host** — the `sites` git remote requires an interactive credential prompt (`git fetch sites` on 2026-09-15 eventually failed with `could not read Username for 'https://git.chatgpt-team.site'`) that a non-interactive session cannot answer, so this remains open pending either an interactive session/stored credential or the user running the check directly.
...
- `road-route-status`: an aria-live helper inside `connection-control` with `idle`, `loading`, `ready`, `needs-route`, `configuration-error`, and `upstream-error` states. It displays road distance and estimated duration only after the route response validates. On the canonical GitHub Pages deployment this surfaces as a generic request failure rather than `configuration-error`, since no `/api/directions` route exists there at all (see section 1) — this is a known, accepted gap on that deployment target, not a defect in this state list.
...
- Road-route connections do not work on the canonical GitHub Pages deployment (static hosting has no `/api/directions` route) and are unverified against the real Sites worker deployment (see section 1). This is tracked open, not a target for client-side workaround code.
